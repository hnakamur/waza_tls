...
fn decryptAndCheck(
    ...
    {
        const c_bytes = try bigint.constToBytesBig(allocator, c);
        defer allocator.free(c_bytes);
        std.log.debug("decryptAndCheck c={}", .{std.fmt.fmtSliceHexLower(c_bytes)});
    }

    var m = try decrypt(priv_key, allocator, random, c);
    errdefer bigint.deinitConst(m, allocator);
    {
        const m_bytes = try bigint.constToBytesBig(allocator, m);
        defer allocator.free(m_bytes);
        std.log.debug("decryptAndCheck m={}", .{std.fmt.fmtSliceHexLower(m_bytes)});
    }
    ...
    {
        const check_bytes = try bigint.constToBytesBig(allocator, check);
        defer allocator.free(check_bytes);
        std.log.debug("decryptAndCheck check={}", .{std.fmt.fmtSliceHexLower(check_bytes)});
    }
...
fn decrypt(
    ...
    if (random) |rand| {
        ...
        c2 = c_copy.toConst();
        {
            const c2_bytes = try bigint.constToBytesBig(allocator, c2);
            defer allocator.free(c2_bytes);
            std.log.debug("rsa.decrypt c2={}", .{std.fmt.fmtSliceHexLower(c2_bytes)});
        }
    }
...
fn signPss(
    ...
    random.bytes(salt);
    std.log.debug("signPss salt={}", .{std.fmt.fmtSliceHexLower(salt)});
...
fn signPssWithSalt(
    ...
    std.log.debug("signPssWithSalt, em_bits={}", .{em_bits});
    var em = try emsaPssEncode(allocator, hashed, em_bits, salt, hash_type);
    defer allocator.free(em);
    std.log.debug("signPssWithSalt em={}", .{std.fmt.fmtSliceHexLower(em)});
...
fn emsaPssEncode(
    ...
    try mgf1Xor(allocator, db, hash_type, h);
...
fn mgf1Xor(allocator: mem.Allocator, out: []u8, hash_type: HashType, seed: []const u8) !void {
    var counter = [_]u8{0} ** 4;
    var digest = try allocator.alloc(u8, hash_type.digestLength());
...
test "mgf1Xor" {
    ...
    try mgf1Xor(allocator, out, .sha256, seed);
...
fn emsaPssVerify(
    ...
    //     stop.
    const bit_mask = 0xff >> (8 * em_len - em_bits);
    if (em[0] & ~bit_mask != 0) {
        return error.Verification;
    }
    ...
    var h0 = hash.allocFinal(allocator);
...
test "rsa.signPss" {
    ...
    defer priv_key.deinit(allocator);
    {
        ...
        std.log.debug("priv_key.d={}", .{std.fmt.fmtSliceHexLower(d_bytes)});
    }
