...
fn expNn(
    ...
    if (x_abs.order(one) == .gt and y_abs_limbs_len > 1 and !m_abs.eqZero()) {
        ...
        errdefer out.deinit();
        try expNnWindowed2(&out, x_abs, y_abs, m_abs);
        {
            ...
            std.log.debug(
                "expNnWindowed, x_abs={}, y_abs={}, m_abs={}, out={}",
...
fn expNnWindowed(
    ...
    std.log.debug("expNnWindowed start", .{});
...
fn expNnWindowed2(
    ...
    std.log.debug("expNnWindowed2 start", .{});
...
test "expNnWindowed" {
    ...
    const f = struct {
        fn f(want: Managed, x: Const, y: Const, m: Const) !void {
            ...
            var got = try expNnWindowed(allocator, x, y, m);
            defer got.deinit();
