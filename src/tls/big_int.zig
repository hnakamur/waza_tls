...
pub fn expConst(
    ...
    std.log.debug("after expNn, len={}, limbs.len={}, metadata={}, limbs={}", .{
        z.len(),                                           z.limbs.len, z.metadata,
        std.fmt.fmtSliceHexLower(managedToBytesLittle(z)),
    });
    z.setSign(!(!z.eqZero() and !x.positive and !y.eqZero() and y.limbs[0] & 1 == 1));
    std.log.debug("z isPositive={}", .{z.isPositive()});
    if (!z.isPositive() and !m.eqZero()) {
        std.log.debug("if true", .{});
        // make modulus result positive
        // z == x**y mod |m| && 0 <= z < |m|
        // try z.ensureAddCapacity(m_abs, z.toConst().abs());
        {
            var m_abs_m = try m_abs.toManaged(allocator);
            defer m_abs_m.deinit();
            std.log.debug("before sub, m_abs={}, z={}", .{ m_abs_m, z });
        }
        try sub(&z, m_abs, z.toConst().abs());
        {
            std.log.debug("after sub, z={}", .{z});
        }
        return z.toConst().abs();
    }
    std.log.debug("if false", .{});
    return z.toConst();
}
...
fn expNn(
    ...
    if (x_abs.order(one) == .gt and y_abs_limbs_len > 1 and !m_abs.eqZero()) {
        if (m_abs.limbs[0] & 1 == 1) {
            var out2 = try Managed.init(allocator);
            // errdefer out2.deinit();
            try expNnMontgomery(&out2, x_abs, y_abs, m_abs);
            std.log.debug("after expNnMontgomery", .{});
            return out2;

            // var out = try expNnMontgomeryOld(allocator, x_abs, y_abs, m_abs);
            // {
            //     var x_abs_m = try x_abs.toManaged(allocator);
            //     defer x_abs_m.deinit();
            //     var y_abs_m = try y_abs.toManaged(allocator);
            //     defer y_abs_m.deinit();
            //     var m_abs_m = try m_abs.toManaged(allocator);
            //     defer m_abs_m.deinit();
            //     std.log.debug(
            //         "expNnMontgomery, x_abs={}, y_abs={}, m_abs={}, out={}",
            //         .{ x_abs_m, y_abs_m, m_abs_m, out },
            //     );
            // }
            // return out;
        }
...
test "expConst" {
    ...
    // bad
    // [default] (debug): after expNn, len=32, limbs.len=32, metadata=32, z=
    // good
    // [default] (debug): after expNn, len=32, limbs.len=32, metadata=32, z=

    // bad
    // [default] (debug): after expNn, len=32, limbs.len=32, metadata=32, limbs=
    // good
    // [default] (debug): after expNn, len=32, limbs.len=32, metadata=32, limbs=
    try f(
        16,
