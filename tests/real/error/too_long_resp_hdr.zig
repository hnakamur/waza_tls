...
test "real / error / too long resp header" {
    ...
    try struct {
        ...
        const Handler = struct {
            ...
            pub fn sendResponse(self: *Handler) void {
                ...
                std.mem.set(u8, self.conn.send_buf[fbs.pos..], 'a');
                self.conn.sendFull(self.conn.send_buf, sendResponseCallback);
            }
        ...
        fn runTest() !void {
            ...
            try testing.expectError(error.HeaderTooLong, self.recv_header_result);
        }
    }.runTest();
}
