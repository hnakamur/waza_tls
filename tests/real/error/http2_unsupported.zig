...
test "real / error / http2 unsupported" {
            ...
            pub fn recvRequestHeaderCallback(self: *Handler, result: Server.RecvRequestHeaderError!usize) void {
                ...
                if (result) |_| {
                    ...
                    if (err != error.HttpVersionNotSupported) {
                        std.log.err("Handler.recvRequestHeaderCallback should get error.HttpVersionNotSupported, found={s}", .{@errorName(err)});
                    }
                }
            }
