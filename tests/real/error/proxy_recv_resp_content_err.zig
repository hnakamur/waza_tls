...
test "real / error / recv resp content error" {
    ...
    try struct {
        ...
        const Handler = struct {
            ...
            pub fn deinit(self: *Handler) void {
                if (self.received_content) |c| {
                    self.conn.server.allocator.free(c);
                }
            }
            ...
            fn closeCallback(
                ...
                std.log.debug("OriginServer.Handler.closeCallback result={}", .{result});
                self.conn.server.done = true;
        ...
        fn recvResponseContentFragmentCallback(
            ...
            if (result) |received| {
                ...
                std.log.warn("recvResponseContentFragmentCallback err={s}", .{@errorName(err)});
                self.exitTestWithError(err);
            }
        }
        ...
        fn runTest() !void {
            ...
            try testing.expectEqual(@as(?u64, self.content.len), self.response_content_length);
            try testing.expectEqualStrings(self.content[0 .. self.content.len / 2], self.received_content.?[0 .. self.content.len / 2]);
