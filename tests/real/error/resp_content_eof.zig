...
test "real / error / resp content eof" {
    ...
    try struct {
        ...
        const Handler = struct {
            ...
            pub fn sendResponse(self: *Handler) void {
                ...
                std.fmt.format(w, "\r\n", .{}) catch unreachable;
                std.fmt.format(w, "{s}", .{"incomplete content"}) catch unreachable;
        ...
        recv_content_result: Client.RecvResponseContentFragmentError!usize = undefined,
        ...
        fn runTest() !void {
            ...
            try testing.expectError(error.UnexpectedEof, self.recv_content_result);
        }
    }.runTest();
}
